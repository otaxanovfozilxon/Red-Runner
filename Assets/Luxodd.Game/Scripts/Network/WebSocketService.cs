...
namespace Luxodd.Game.Scripts.Network
{
    public class WebSocketService : MonoBehaviour
    {
        ...
        private async Task StartConnectionAsync()
        {
            ...
            if (string.IsNullOrEmpty(serverUrlRaw) && !shouldUseHost && _sessionPayload == null)
            {
                Debug.LogError($"[{DateTime.Now}][{GetType().Name}][{nameof(StartConnectionAsync)}] No valid WebSocket URL available. serverUrlRaw='{serverUrlRaw}', shouldUseHost={shouldUseHost}");
                _isConnected = false;
