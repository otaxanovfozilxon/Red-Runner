...
namespace Luxodd.Game.Scripts.Network.CommandHandler
{
    ...
    public class WebSocketCommandHandler : MonoBehaviour
    {
        ...
        {
            // Avoid Enum.GetValues().Cast<T>().ToList() — LINQ generic state machines
            // cause IL2CPP function signature mismatch in WebGL builds
            var allValues = (CommandRequestType[])Enum.GetValues(typeof(CommandRequestType));
            var commandTypes = new List<CommandRequestType>(allValues.Length);
            for (int i = 0; i < allValues.Length; i++)
            {
                commandTypes.Add(allValues[i]);
            }
